...
from model.cmrclip import sim_matrix
...
class Trainer(BaseTrainer):
    ...
    def __init__(self, model, loss, metrics, optimizer, config, data_loader,
                 ...
                 visualizer=None, tokenizer=None, tokenizer_cxr=None, max_samples_per_epoch=50000):
        ...
        self.tokenizer_cxr = tokenizer_cxr
    ...
    def _train_epoch(self, epoch):
        ...
        with tqdm(zip(*self.data_loader), desc=f"Training epoch {epoch}", total=total_iterations, disable=False) as progress:
            for batch_idx, data_li in enumerate(progress):
                ...
                for dl_idx, data in enumerate(data_li):
                    ...
                    if self.tokenizer_cxr is not None:
                        for txt_idx in range(len(data['text'])):
                            data['text'][txt_idx] = self.tokenizer_cxr(data['text'][txt_idx])
                            data['text'][txt_idx] = self.tokenizer_cxr.decode(data['text'][txt_idx])
                    ...
                    if self.use_amp:
                        with autocast():
                            text_projection, video_projection = self.model(data)
                            loss = self.loss(text_projection, video_projection)
    ...
    def _valid_epoch(self, epoch):
        ...
        with torch.no_grad():
            ...
            for dl_idx, dl in enumerate(self.valid_data_loader):
                for data in tqdm(dl, desc=f"Validating dl{dl_idx}", disable=False):
                    ...
                    if avoid_data_parallel:
                        text_projection, video_projection = self.model.module(data)
                    else:
                        text_projection, video_projection = self.model(data)

                    text_embed_arr[dl_idx].append(text_projection.cpu())
                    vid_embed_arr[dl_idx].append(video_projection.cpu())
                    loss = self.loss(text_projection, video_projection)
