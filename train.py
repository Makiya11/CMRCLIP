...
import model.cmrclip as module_arch
...
def run():
    ...
    if 'lr_scheduler' in config._config:
        ...
        warmup_sched = LinearLR(
            optimizer,
            start_factor=config['lr_scheduler']['warmup_start_lr'] / config['optimizer']['args']['lr'],
            end_factor=1.0,
            total_iters=config['lr_scheduler']['warmup_epochs']
        )
        cosine_sched = CosineAnnealingLR(
            optimizer,
            T_max=config['trainer']['epochs'] - config['lr_scheduler']['warmup_epochs'],
            eta_min=config['lr_scheduler']['eta_min']
        )
        lr_scheduler = SequentialLR(
            optimizer,
            schedulers=[warmup_sched, cosine_sched],
            milestones=[config['lr_scheduler']['warmup_epochs']]
        )
    else:
        lr_scheduler = None
